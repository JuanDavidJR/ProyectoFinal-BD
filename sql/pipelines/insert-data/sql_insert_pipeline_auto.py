...
class SQLPipeline:
    def __init__(self):
        ...
        '01-genres.sql',        # Independent table
        '02-artists.sql',       # Independent table
        '06-users.sql',         # Independent table
        '07-devices.sql',       # Depends on user_devices
        '03-albums.sql',        # Depends on artists
        '04-music.sql',         # Depends on albums
        '05-song-genres.sql',   # Depends on songs and genres
        '08-user-device.sql',   # Depends on users and devices
        '09-playlists.sql',     # Depends on users
        '10-playlist-song.sql', # Depends on playlists and songs
        '11-reproductions.sql'  # Depends on users, songs, and devices
]
    ...
    def parse_arguments(self):
        ...
        parser.add_argument('--db-name', default='musicdb',
                          help='Database name (default: musicdb)')
        parser.add_argument('--schema-name', default='vibesia_schema',
                          help='Schema name (default: vibesia_schema)')
    ...
    def run(self):
        ...
        try:
            ...
            #Add bulk load SQL files (in Bulk-Load directory) "YOU NEED TO DESCOMENT THIS TO LOAD BULK FILES "
...
def get_project_root():
    """Returns the absolute path to the musicdb directory"""
